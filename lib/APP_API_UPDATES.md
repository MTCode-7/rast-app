...
## جديد: سلة التحاليل + دفع واحد

راجع الملف الكامل: **[`CART_API.md`](CART_API.md)**

| الخطوة | API |
|--------|-----|
| إضافة تحليل | `POST /api/cart/items` |
| عرض السلة | `GET /api/cart` |
| إتمام الطلب | `POST /api/cart/checkout` → `cart_order_id` |
| الدفع | `POST /api/cart-orders/{id}/payment/session` |
| بعد WebView | `GET /api/cart-orders/{id}/payment/status` |

- الحجز المفرد القديم **ما زال يعمل**.
- لا تدفع حجزاً له `cart_order_id` منفرداً — ادفع طلب السلة.
...
| الأولوية | المهمة |
|----------|--------|
| **عاجل** | تنفيذ السلة + الدفع المجمّع (`CART_API.md`) |
