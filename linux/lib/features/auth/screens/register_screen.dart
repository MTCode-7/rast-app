...
class _RegisterScreenState extends State<RegisterScreen> {
    ...
    return Column(
      children: [
        Container(
          ...
          child: Icon(Icons.person_add_rounded, size: 44, color: Colors.white.withValues(alpha: 0.95)),
        ),
        SizedBox(height: Responsive.spacing(context, 18)),
        Text(
          'إنشاء حساب جديد',
          style: TextStyle(fontSize: Responsive.fontSize(context, 24), fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: -0.3),
  ...
  Widget _buildFormCard() {
    ...
    final fillColor = isDark ? const Color(0xFF0F172A) : AppTheme.surfaceVariant.withValues(alpha: 0.5);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: Colors.black.withValues(alpha: isDark ? 0.3 : 0.1), blurRadius: 28, offset: const Offset(0, 12)),
        ],
      ),
      child: Form(
        key: _formKey,
  ...
  Widget _buildLoginLink() {
    return Row(
      ...
      children: [
        Text('لديك حساب بالفعل؟ ', style: TextStyle(color: Colors.white.withValues(alpha: 0.9), fontSize: Responsive.fontSize(context, 14))),
        TextButton(
          onPressed: () => Navigator.pop(context),
          style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 8)),
          child: Text('تسجيل الدخول', style: TextStyle(color: AppTheme.accent, fontWeight: FontWeight.w700, fontSize: Responsive.fontSize(context, 15))),
        ),
      ],
    );
  }
