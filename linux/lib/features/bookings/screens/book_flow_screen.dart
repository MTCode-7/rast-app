...
class _BookFlowScreenState extends State<BookFlowScreen> {
  ...
  String get _serviceNameAr => (widget.providerService['service'] is Map
      ...
      ?.toString() ??
      '';
  double get _price => (widget.providerService['final_price'] ?? widget.providerService['price'] ?? 0) is num
      ? (widget.providerService['final_price'] ?? widget.providerService['price'] as num).toDouble()
      : 0.0;
  double get _homePrice => (widget.providerService['home_service_price'] ?? widget.providerService['home_service_price']) is num
      ? (widget.providerService['home_service_price'] as num).toDouble()
      : _price + 25;
  ...
  double get _totalAmount {
    if (_serviceType == 'home_service') return _homePrice;
    return _price;
  }
  ...
  Widget _buildStepServiceType() {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: Responsive.spacing(context, 12)),
        _serviceTypeChip('in_clinic', 'في المختبر', Icons.business_rounded, '$_price ر.س'),
        if (_homeAvailable) ...[
          SizedBox(height: Responsive.spacing(context, 10)),
          _serviceTypeChip('home_service', 'زيارة منزلية', Icons.home_rounded, '$_homePrice ر.س'),
        ],
        if (_serviceType == 'home_service') ...[
          SizedBox(height: Responsive.spacing(context, 20)),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _confirmRow('المبلغ الإجمالي', '$_totalAmount ر.س', valueStyle: TextStyle(color: AppTheme.primary, fontWeight: FontWeight.bold)),
            ],
          ),
        ),
      ],
