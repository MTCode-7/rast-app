...
class _LabsScreenState extends State<LabsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        _error = e.message;
        _labs = List.from(DummyData.labs);
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('المختبرات', style: TextStyle(fontSize: Responsive.fontSize(context, 16), fontWeight: FontWeight.w500)),
  ...
  Widget _buildSortRow() {
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          Text('ترتيب: ', style: TextStyle(fontSize: Responsive.fontSize(context, 12), color: AppTheme.onSurfaceVariant)),
          SizedBox(width: 8),
  ...
  Widget _sortChip(String label, String value) {
    ...
    return FilterChip(
      label: Text(label, style: TextStyle(fontSize: Responsive.fontSize(context, 12), fontWeight: FontWeight.w400)),
      selected: selected,
      onSelected: (_) {
        setState(() {
          _sortBy = value;
...
class _LabCard extends StatelessWidget {
  ...
  Widget _buildLogo(String? url, double size) {
    return Container(
      width: size,
      height: size,
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(18), boxShadow: AppTheme.cardShadow, border: Border.all(color: Colors.white.withValues(alpha: 0.9), width: 1)),
      child: ClipRRect(
