...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  Widget _buildBookingBreakdown(BuildContext context) {
    ...
    double servicePrice = _parseNum(booking['service_price']);
    double homeFee = _parseNum(booking['home_service_fee']);
    double discountAmount = _parseNum(booking['discount_amount']);
    double vatAmount = _parseNum(meta?['vat_amount']);

    if (servicePrice == 0 && psMap != null) {
      servicePrice = ApiConfig.priceFromMap(psMap);
    }
    if (isHomeService && homeFee == 0 && psMap != null) {
      homeFee = _parseNum(psMap['home_service_price']);
    }
    if (discountAmount == 0) {
    ...
    double totalAmount = _parseNum(booking['total_amount']);
    if (totalAmount == 0) {
        ...
        _buildSection(context, 'التقييم', [
          if (hasReview) ...[
            ...
            FilledButton.icon(
              ...
              style: FilledButton.styleFrom(backgroundColor: AppTheme.primary, padding: EdgeInsets.symmetric(vertical: 14)),
            ),
        ]),
      ],
    );
  ...
  Future<void> _showReviewDialog(BuildContext context) async {
    ...
    if (bid == null) return;
    try {
      await Api.bookings.submitReview(
        bid is int ? bid : int.parse(bid.toString()),
        rating,
        comment: controller.text.trim().isEmpty ? null : controller.text.trim(),
      );
      if (mounted) {
        final fresh = await Api.bookings.getBooking(bid is int ? bid : int.parse(bid.toString()));
        setState(() => _booking = _mergeBooking(_booking, fresh));
