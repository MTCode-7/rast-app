...
class _BookFlowScreenState extends State<BookFlowScreen> {
  ...
  String get _serviceNameAr => (widget.providerService['service'] is Map
      ...
      '';
  double get _price {
    final v = widget.providerService['final_price'] ?? widget.providerService['price'];
    if (v is num) return v.toDouble();
    if (v != null) return double.tryParse(v.toString()) ?? 0.0;
    return ApiConfig.priceFromMap(widget.providerService);
  }

  /// رسوم إضافية للزيارة المنزلية (تُضاف إلى سعر التحليل في المختبر)
  double get _homeServiceFeeRaw {
    final v = widget.providerService['home_service_price'];
    if (v is num) return v.toDouble();
    if (v != null) return double.tryParse(v.toString()) ?? 25;
    return 25;
  }
  ...
  double get _platformDiscountRate =>
      (_bookingConfig?['platform_discount_rate'] is num
          ? (_bookingConfig!['platform_discount_rate'] as num).toDouble()
          : null) ??
      ApiConfig.globalDiscountPercent.toDouble();

  double get _vatRate =>
      (_bookingConfig?['vat_rate'] is num
          ? (_bookingConfig!['vat_rate'] as num).toDouble()
          : null) ??
      0.0;
  ...
  double get _discountAmount =>
      (_price * (_platformDiscountRate / 100)).roundToDouble();
  ...
  Map<String, dynamic> _toBookingMap(Map<String, dynamic> booking) {
    ...
    return {
      ...
      'total_amount': booking['total_amount'] ?? _totalAmount,
  ...
  Widget _buildSuccessBody() {
    ...
    final servicePrice = _parseNum(_createdBooking?['service_price']);
    final homeFee = _parseNum(_createdBooking?['home_service_fee']);
    final discountAmount = _parseNum(_createdBooking?['discount_amount']);
    final totalAmount = _parseNum(_createdBooking?['total_amount']);
    final meta = _createdBooking?['metadata'];
    final vatAmount = meta is Map ? _parseNum(meta['vat_amount']) : 0.0;
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: Responsive.spacing(context, 10)),
              _confirmRow('سعر التحليل (في المختبر)', '${_price.toStringAsFixed(2)} ر.س'),
              if (_serviceType == 'home_service')
                _confirmRow('رسوم الخدمة المنزلية (إضافي)', '+ ${_homeServiceFee.toStringAsFixed(2)} ر.س'),
              ...
              if (_isNonSaudi)
                _confirmRow('ضريبة القيمة المضافة ${_vatRate.toStringAsFixed(0)}% (غير سعودي)', '+ ${_vatAmount.toStringAsFixed(2)} ر.س', valueStyle: TextStyle(color: AppTheme.onSurfaceVariant)),
              Padding(
